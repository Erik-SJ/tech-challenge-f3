...
if not DATABASE_URL or not AUTH_SERVICE_URL:
    log.critical(
        "Erro: DATABASE_URL e AUTH_SERVICE_URL "
        "devem ser definidos."
    )
    sys.exit(1)
...
try:
    ...
    log.critical(
        f"Erro fatal ao conectar ao PostgreSQL: {exc}"
    )
    sys.exit(1)
...
def get_flags():
    ...
    try:
        ...
        cur.execute(
            "SELECT * FROM flags ORDER BY name"
        )
...
def get_flag(name):
    ...
    try:
        ...
        cur.execute(
            "SELECT * FROM flags WHERE name = %s",
            (name,),
        )
        ...
        log.error(
            f"Erro ao buscar flag '{name}': {exc}"
        )
...
def update_flag(name):
    """Atualiza descrição ou status da feature flag."""
    ...
    try:
        ...
        log.error(
            f"Erro ao atualizar flag '{name}': {exc}"
        )
...
def delete_flag(name):
    ...
    try:
        ...
        cur.execute(
            "DELETE FROM flags WHERE name = %s",
            (name,),
        )
        ...
        log.error(
            f"Erro ao deletar flag '{name}': {exc}"
        )
