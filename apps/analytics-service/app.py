...
def process_message(message):
    """Processa uma única mensagem SQS e insere no DynamoDB."""
    try:
        log.info(
            f"Processando mensagem ID: {message['MessageId']}"
        )
        ...
        log.error(
            "Erro ao decodificar JSON da mensagem ID: "
            f"{message['MessageId']}"
        )
...
def sqs_worker_loop():
    ...
    while True:
        try:
            ...
            log.info(
                f"Recebidas {len(messages)} mensagens."
            )
            ...
            log.error(
                f"Erro do Boto3 no loop principal do SQS: {e}"
            )
            time.sleep(10)  # Pausa antes de tentar novamente

        except Exception as e:
            log.error(
                f"Erro inesperado no loop principal do SQS: {e}"
            )
            time.sleep(10)
...
@app.route("/health")
def health():
    # Uma verificação de saúde real poderia checar
    # a conexão com o DynamoDB/SQS
...
# Isso garante que ele inicie tanto com "flask run"
# quanto com "gunicorn"
start_worker()
